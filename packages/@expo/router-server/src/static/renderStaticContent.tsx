...
import { createHeadInjectionTransform } from '../utils/streams';
...
export async function getStreamingContent(
  ...
  const css = ReactDOMServer.renderToStaticMarkup(getStyleElement());
  const { headTags } = serializeHelmetToHtml(headContext.helmet);
  const fonts = Font.getServerResources();
  debug(`Pushing static fonts: (count: ${fonts.length})`, fonts);
  ...
  return stream.pipeThrough(createHeadInjectionTransform(injectionParts));
}
